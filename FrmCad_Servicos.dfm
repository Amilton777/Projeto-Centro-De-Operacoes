object frm_nova_ordem_servico: Tfrm_nova_ordem_servico
  Left = 0
  Top = 0
  BorderIcons = [biSystemMenu]
  Caption = 'CADASTRO DE ORDEM DE SERVI'#199'O'
  ClientHeight = 473
  ClientWidth = 1095
  Color = clMenu
  Font.Charset = ANSI_CHARSET
  Font.Color = clNavy
  Font.Height = -13
  Font.Name = 'Trebuchet MS'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poScreenCenter
  OnClose = FormClose
  OnCreate = FormCreate
  OnKeyDown = FormKeyDown
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 18
  object Label1: TLabel
    Left = 16
    Top = 18
    Width = 80
    Height = 18
    Caption = 'ID OPERACAO'
  end
  object Label2: TLabel
    Left = 112
    Top = 18
    Width = 64
    Height = 18
    Caption = 'CONTRATO'
  end
  object Label3: TLabel
    Left = 184
    Top = 18
    Width = 312
    Height = 18
    Caption = 'LOGRADOURO COMPLETO (RUA + NUMERO + BAIRRO)'
  end
  object Label4: TLabel
    Left = 640
    Top = 18
    Width = 70
    Height = 18
    Caption = 'DATA INICIO'
  end
  object Label5: TLabel
    Left = 745
    Top = 18
    Width = 111
    Height = 18
    Caption = 'DATA FINALIZA'#199#195'O'
  end
  object Label6: TLabel
    Left = 16
    Top = 114
    Width = 68
    Height = 18
    Caption = 'ID TECNICO'
  end
  object Label7: TLabel
    Left = 112
    Top = 114
    Width = 111
    Height = 18
    Caption = 'NOME DO TECNICO'
  end
  object Label8: TLabel
    Left = 280
    Top = 114
    Width = 63
    Height = 18
    Caption = 'ID ORIGEM'
  end
  object Label9: TLabel
    Left = 376
    Top = 114
    Width = 112
    Height = 18
    Caption = 'ORIGEM DA ORDEM'
  end
  object Label10: TLabel
    Left = 544
    Top = 114
    Width = 65
    Height = 18
    Caption = 'ID SERVI'#199'O'
  end
  object Label11: TLabel
    Left = 640
    Top = 114
    Width = 56
    Height = 18
    Caption = 'SERVI'#199'OS'
  end
  object Label12: TLabel
    Left = 830
    Top = 114
    Width = 62
    Height = 18
    Caption = 'ID PLANOS'
  end
  object Label13: TLabel
    Left = 916
    Top = 114
    Width = 90
    Height = 18
    Caption = 'TIPO DE PLANO'
  end
  object Label14: TLabel
    Left = 897
    Top = 18
    Width = 118
    Height = 18
    Caption = 'STATUS DO SERVI'#199'O'
  end
  object bvl_campos: TBevel
    Left = 8
    Top = 8
    Width = 1079
    Height = 161
    Shape = bsFrame
  end
  object bvl_botoes: TBevel
    Tag = 1
    Left = 8
    Top = 383
    Width = 1079
    Height = 90
    Shape = bsFrame
  end
  object DBEdit1: TDBEdit
    Left = 16
    Top = 42
    Width = 80
    Height = 26
    CharCase = ecUpperCase
    Color = clInfoBk
    DataField = 'idOperacoes'
    DataSource = DS_OPERACAO
    Enabled = False
    TabOrder = 18
  end
  object DBEdT_CONTRATO: TDBEdit
    Left = 112
    Top = 42
    Width = 66
    Height = 26
    CharCase = ecUpperCase
    Color = clInfoBk
    DataField = 'CONTRATO'
    DataSource = DS_OPERACAO
    TabOrder = 0
    OnExit = DBEdT_CONTRATOExit
  end
  object DBEDT_LOGRADOURO: TDBEdit
    Tag = 2
    Left = 184
    Top = 42
    Width = 440
    Height = 26
    CharCase = ecUpperCase
    Color = clInfoBk
    DataField = 'LOGRADOURO'
    DataSource = DS_OPERACAO
    TabOrder = 1
  end
  object DBEdit4: TDBEdit
    Left = 640
    Top = 42
    Width = 89
    Height = 26
    CharCase = ecUpperCase
    Color = clInfoBk
    DataField = 'DATA_INICIAL'
    DataSource = DS_OPERACAO
    TabOrder = 14
  end
  object DBEdit5: TDBEdit
    Left = 745
    Top = 42
    Width = 89
    Height = 26
    CharCase = ecUpperCase
    Color = clInfoBk
    DataField = 'DATA_FINALIZACAO'
    DataSource = DS_OPERACAO
    Enabled = False
    TabOrder = 15
  end
  object DBEdit6: TDBEdit
    Tag = 2
    Left = 16
    Top = 136
    Width = 80
    Height = 26
    CharCase = ecUpperCase
    Color = clInfoBk
    DataField = 'idTecnicos'
    DataSource = DS_OPERACAO
    TabOrder = 3
    OnExit = DBEdit6Exit
  end
  object DBEdit7: TDBEdit
    Left = 280
    Top = 136
    Width = 80
    Height = 26
    CharCase = ecUpperCase
    Color = clInfoBk
    DataField = 'idOrigem'
    DataSource = DS_OPERACAO
    TabOrder = 5
    OnExit = DBEdit7Exit
  end
  object DBEdit8: TDBEdit
    Left = 544
    Top = 136
    Width = 80
    Height = 26
    CharCase = ecUpperCase
    Color = clInfoBk
    DataField = 'idServicos'
    DataSource = DS_OPERACAO
    TabOrder = 7
    OnExit = DBEdit8Exit
  end
  object DBEdit9: TDBEdit
    Left = 830
    Top = 136
    Width = 80
    Height = 26
    CharCase = ecUpperCase
    Color = clInfoBk
    DataField = 'idPlanos'
    DataSource = DS_OPERACAO
    TabOrder = 9
    OnExit = DBEdit9Exit
  end
  object DBEDT_STATUS: TDBEdit
    Left = 846
    Top = 42
    Width = 99
    Height = 26
    CharCase = ecUpperCase
    Color = clInfoBk
    DataField = 'STATUS'
    DataSource = DS_OPERACAO
    Enabled = False
    TabOrder = 16
  end
  object DBEdit11: TDBEdit
    Left = 16
    Top = 84
    Width = 80
    Height = 26
    CharCase = ecUpperCase
    Color = clInfoBk
    DataField = 'TOTAL_PONTOS'
    DataSource = DS_OPERACAO
    TabOrder = 17
    Visible = False
  end
  object CB_TECNICOS: TComboBox
    Tag = 1
    Left = 112
    Top = 136
    Width = 153
    Height = 26
    CharCase = ecUpperCase
    Color = clInfoBk
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlue
    Font.Height = -13
    Font.Name = 'Trebuchet MS'
    Font.Style = []
    ParentFont = False
    TabOrder = 4
    OnChange = CB_TECNICOSChange
  end
  object DBMemo1: TDBMemo
    Left = 10
    Top = 184
    Width = 1077
    Height = 193
    Color = clInfoBk
    DataField = 'OBSERVACAO'
    DataSource = DS_OPERACAO
    TabOrder = 11
  end
  object CB_ORIGEM: TComboBox
    Left = 376
    Top = 136
    Width = 153
    Height = 26
    CharCase = ecUpperCase
    Color = clInfoBk
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlue
    Font.Height = -13
    Font.Name = 'Trebuchet MS'
    Font.Style = []
    ParentFont = False
    TabOrder = 6
    OnChange = CB_ORIGEMChange
    Items.Strings = (
      '')
  end
  object CB_SERVICOS: TComboBox
    Left = 640
    Top = 136
    Width = 161
    Height = 26
    CharCase = ecUpperCase
    Color = clInfoBk
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlue
    Font.Height = -13
    Font.Name = 'Trebuchet MS'
    Font.Style = []
    ParentFont = False
    TabOrder = 8
    OnChange = CB_SERVICOSChange
    Items.Strings = (
      '')
  end
  object CB_TIPO_PLANO: TComboBox
    Left = 916
    Top = 136
    Width = 161
    Height = 26
    CharCase = ecUpperCase
    Color = clInfoBk
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlue
    Font.Height = -13
    Font.Name = 'Trebuchet MS'
    Font.Style = []
    ParentFont = False
    TabOrder = 10
    OnChange = CB_TIPO_PLANOChange
    Items.Strings = (
      '')
  end
  object btn_cancelar: TBitBtn
    Left = 640
    Top = 410
    Width = 209
    Height = 49
    Caption = 'CANCELAR [ESC]'
    Glyph.Data = {
      
      F2F8FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    TabOrder = 13
  end
  object btn_salvar: TBitBtn
    Left = 867
    Top = 410
    Width = 209
    Height = 49
    Caption = 'SALVAR'
    Glyph.Data = {
      
      EFE3FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    TabOrder = 12
    OnClick = btn_salvarClick
  end
  object CB_STATUS: TComboBox
    Tag = 2
    Left = 951
    Top = 42
    Width = 125
    Height = 26
    CharCase = ecUpperCase
    Color = clInfoBk
    TabOrder = 2
    OnChange = CB_STATUSChange
    Items.Strings = (
      '')
  end
  object btn_minha_equipe: TBitBtn
    Left = 112
    Top = 84
    Width = 153
    Height = 29
    Caption = #8595' MINHA EQUIPE  '#8595
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    TabOrder = 19
    OnClick = btn_minha_equipeClick
  end
  object btn_todos: TBitBtn
    Left = 280
    Top = 84
    Width = 177
    Height = 29
    Caption = 'TODOS OS T'#201'CNICOS'
    Glyph.Data = {
      
      7D52B57D52B57D52B57D52B57D52B57D52B57D52FFFFFFFFFFFF}
    TabOrder = 20
    OnClick = btn_todosClick
  end
  object DS_OPERACAO: TDataSource
    DataSet = DM.QRY_Cad_Operacoes
    Left = 608
  end
  object QRY_ORIGEM: TFDQuery
    Connection = DM.FDConnection1
    SQL.Strings = (
      'SELECT * FROM ORIGEM')
    Left = 416
    Top = 72
  end
  object QRY_SERVICOS: TFDQuery
    Connection = DM.FDConnection1
    SQL.Strings = (
      'SELECT DES_SERVICO FROM SERVICOS')
    Left = 648
    Top = 64
  end
  object QRY_TECNICOS: TFDQuery
    Connection = DM.FDConnection1
    SQL.Strings = (
      'Select idTECNICOS, NOME_TECNICO from tecnicos')
    Left = 296
    Top = 392
  end
  object QRY_TIPO_PLANO: TFDQuery
    Connection = DM.FDConnection1
    SQL.Strings = (
      'SELECT DESC_PLANO FROM PLANOS')
    Left = 752
    Top = 72
  end
  object QRY_STATUS: TFDQuery
    Connection = DM.FDConnection1
    SQL.Strings = (
      'SELECT STATUS_OS FROM STATUS'
      'WHERE STATUS_OS <> '#39'FINALIZADO'#39)
    Left = 872
    Top = 72
  end
  object QRY_BUSCA_DADOS: TFDQuery
    Connection = DM.FDConnection1
    SQL.Strings = (
      'select * from facilidade')
    Left = 544
    Top = 32
  end
end
