...
begin
  // Pegando posição no combobox ORIGEM e atribuindo.
  DBEdit7.Text:=IntToStr((CB_ORIGEM.Items.IndexOf(CB_ORIGEM.text)));
end;
...
begin
  ...
  // Pegando posição no combobox SERVIÇOS e atribuindo.
  DBEdit8.Text:=IntToStr((CB_SERVICOS.Items.IndexOf(CB_SERVICOS.text)));
  ...
  IF ((INDEX_COMBO_SERVICOS = 3) OR (INDEX_COMBO_SERVICOS = 4) or (INDEX_COMBO_SERVICOS=5)or (INDEX_COMBO_SERVICOS=6)) then
    Begin
      ...
      //Se for INSTALAÇÃO OU TRANSFERENCIA ATRIBUIR ORIGEM COMERCIAL
...
begin
  ...
  // Pegando posição no combobox TIPO_DE_PLANO e atribuindo.
  DBEdit9.Text:=IntToStr((CB_TIPO_PLANO.Items.IndexOf(CB_TIPO_PLANO.text)));
end;
...
begin
    Try
      //Selecionando o endereço automaticamente a partir de dados herdados tabela FACILIDADE
      ...
      //Adicionando alguns dados herdados no campo de observação.
...
begin
    //Caso o Usuario feche o form sem preencher o contrato OR o endereço deletar o registro.
...
begin
      ...
      // Trazer os dados para o combox Técnicos
      QRY_TECNICOS.Close;
      QRY_TECNICOS.SQL.Clear;
    ...
    IF QRY_ORIGEM.RowsAffected > 0 then
      ...
      // Trazer os dados para o combox Serviços
     QRY_SERVICOS.Close;
...
begin
    ...
    //Inicializando o forma con data e outras informações padrões
    DS_OPERACAO.DataSet.Open;
    DS_OPERACAO.DataSet.Insert;
